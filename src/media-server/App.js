...
export default class MediaServerApp extends Core {
    ...
    get_live(id) {
        ...
        if (this.sessions[id] && this.sessions[id].live) return this.lives[this.sessions[id].live.id];
    }
...
export class Live extends DataNode {
    /** @type {FFMPEGWrapper} */
    ffmpeg;
    ...
    constructor(id) {
        ...
        this.dir = path.join(globals.app.live_dir, id);
        ...
        this.player_url = `${globals.app.get_urls("media-server").url}/player/index.html?id=${this.id}`;

    }

    async load() {
        var d;
        try { d = JSON.parse(fs.readFileSync(this.data_filename, "utf-8")); } catch {};
        if (d) await this.init(d);
    }
    ...
    async init(opts, session) {
        ...
        if (session) {
            ...
            session.live = {
                id: this.id,
                url: this.player_url,
                origin: session.publishArgs.origin,
            };
        }
        ...
        var fix_outputs = (outputs)=>{
            (outputs && outputs.length) ? outputs : utils.deep_copy(globals.app.conf["media-server.outputs"]);
    ...
    async create_thumbnail() {
        ...
        this.session.live.thumbnail_url = `${this.url}/thumbnails/${thumbnail_name}`;
    };
    ...
    async fetch(req, res) {
        ...
        if (this.#running) {
            ...
            if (await fs.exists(f)) {
                ...
                fs.createReadStream(path.join(this.live_dir, `${id}/${v}.vod.m3u8`), {encoding:'utf8'}).pipe(res);
                return true;
            }
        }
    }
...
export class LiveLevel extends events.EventEmitter {
    ...
    async #add_segment(segment) {
        ...
        var stat = await fs.stat(path.join(this.dir, segment.uri)).catch(()=>{});
        if (stat) {
