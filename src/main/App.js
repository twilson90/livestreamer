...
/** @import {StreamTarget, SessionBase} from "./types.d.ts" */
...
var format_ext_map = utils.reverse_map(ext_format_map);
...
export default class MainApp extends Core {
    ...
    async init() {
        ...
        this.ipc.on("media-server.done-publish", (id)=>{
            Object.values(this.sessions).filter(s=>s instanceof ExternalSession && s.nms_session && s.nms_session.id == id).forEach(s=>s.destroy());
    ...
    probe_media(filename, opts) {
        ...
        var is_new = !this.$.media_info[filename];
        if (is_new) this.$.media_info[filename] = {};
        let data = this.$.media_info[filename];
        if (!opts.silent && (is_new || opts.force)) data.processing = 1;
        ...
        return this.#media_info_promise_pool.enqueue(async ()=>{
            ...
            let new_data = (opts.force) ? null : await this.#media_info_cache.get(cache_key);
            ...
            if (!new_data) {
                ...
                check: {
                    ...
                    if (protocol === "file:") {
                        ...
                        if (stat) {
                            new_data.size = stat.size;
                            new_data.mtime = stat.mtimeMs;
                            ...
                            if (!is_edl_file) {
                                ...
                                if (is_playlist_file) {
                                    ...
                                    if (rmi) {
                                        ...
                                        new_data.duration = parseFloat(rmi.format.duration) || 0;
                                        // let orig_duration = info.duration;
                                        new_data.chapters = rmi.chapters.map((c,i)=>({ index: i, start: +c.start_time, end: +c.end_time, title: (c.tags) ? c.tags.title : null }));
                                        new_data.format = rmi.format.format_name;
                                        new_data.bitrate = +rmi.format.bit_rate || 0;
                                        new_data.streams = rmi.streams.map(s=>{
                                            let stream = {};
                    ...
                    if (protocol === "http:" || protocol === "https:") {
                        ...
                        new_data.exists = !!rmi;
                        if (rmi) {
                            new_data.size = +rmi.filesize_approx;
                            // data.atime = data.mtime = data.ctime = data.btime = +rmi.epoch;
                            new_data.name = rmi.is_playlist ? rmi.items[0].playlist_title : rmi.fulltitle;
                            new_data.filename = rmi._filename;
                            new_data.downloadable = true;
                            new_data.direct = !!rmi.direct;
                            if (rmi.is_playlist) {
                                new_data.playlist = rmi.items.map(i=>i.url || i.webpage_url);
                            } else {
                                new_data.duration = rmi.duration;
                                new_data.streams = [
                                    {
                                        type: "video",
                                        bitrate: rmi.vbr,
                    ...
                    if (protocol === "edl:" || is_edl_file) {
                        ...
                        if (rmi) {
                            // mi.name = filename;
                            new_data.duration = rmi.duration;
                            new_data.streams = rmi["track-list"].map(t=>{
                                let stream = {};
                ...
                if (new_data && protocol) new_data.protocol = protocol;
            ...
            utils.deep_sync(data, new_data);
            this.#media_info_cache.set(cache_key, new_data, ttl);
    ...
    parse_targets(targets) {
        if (typeof targets === "string") {
            ...
            if (t === undefined) t = targets.split(",");
            targets = t;
        }
