...
var web_configs = [ "media-server", "file-manager", "main" ]
    .flatMap((input)=>{
        var index = path.resolve(`src/${input}/public_html/player/index.html`);
...
export async function build() {
    for (var config of [node_config, ...web_configs]) {
        await vite.build(config);
    }
}

export async function dev() {
    for (var config of [node_config]) {
